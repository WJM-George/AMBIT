...
def load_ckpt_state_dict(ckpt_path, *, return_metadata=False):
    metadata = {}
    if ckpt_path.endswith(".safetensors"):
        state_dict = load_file(ckpt_path)
    else:
        try:
            # Multi-GiB Lightning checkpoints also contain optimizer/EMA state.
            # mmap avoids paging every tensor into RAM when a route warm-start
            # only touches the model weights it can reuse.
            ...
            if "mmap" not in str(error).lower():
                raise
            # Compatibility with legacy, non-zip torch.save checkpoints.
            checkpoint = torch.load(
        ...
        if isinstance(checkpoint.get("text_conditioner_ema_parameter_names"), list):
            metadata["text_conditioner_ema_parameter_names"] = checkpoint[
                "text_conditioner_ema_parameter_names"
            ]
        if isinstance(checkpoint.get("conditioner_ema_parameter_names"), list):
            metadata["conditioner_ema_parameter_names"] = checkpoint[
