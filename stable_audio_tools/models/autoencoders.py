...
from .foa_spar import FOALatentSPARDecoder
...
class AudioAutoencoder(nn.Module):
    def __init__(
        ...
        latent_spar: FOALatentSPARDecoder = None,
    ):
        super().__init__()  
        ...
        self.latent_spar = latent_spar
        if self.latent_spar is not None and self.out_channels != 4:
            raise ValueError("latent_spar requires a four-channel FOA decoder")
        if self.latent_spar is not None and soft_clip:
            raise ValueError(
                "latent_spar requires soft_clip=False so [W, residual] synthesis remains linear"
            )
    ...
    def decode(
        ...
        **kwargs,
        ...
        spar_info = None
        if self.latent_spar is not None:
            if return_spar_info:
                decoded, spar_info = self.latent_spar(
                    decoded, decoder_latents, return_info=True
                )
            else:
                decoded = self.latent_spar(decoded, decoder_latents)
        elif return_spar_info:
            raise RuntimeError("return_spar_info=True requires latent_spar to be configured")

        if self.soft_clip:
            decoded = torch.tanh(decoded)
        if return_loss and return_spar_info:
            return decoded, loss, spar_info
        elif return_loss:
            return decoded, loss
        elif return_spar_info:
            return decoded, spar_info
        else:
            return decoded
...
def create_autoencoder_from_config(config: Dict[str, Any]):
    ...
    latent_spar = None
    latent_spar_config = ae_config.get("latent_spar")
    if latent_spar_config is not None:
        latent_spar_enabled = latent_spar_config.get("enabled", False)
        if not isinstance(latent_spar_enabled, bool):
            raise ValueError("model.latent_spar.enabled must be a boolean")
        if latent_spar_enabled:
            if latent_spar_config.get("type") != "foa_latent_spar":
                raise ValueError(
                    "model.latent_spar must use the 'foa_latent_spar' type"
                )
            latent_spar = FOALatentSPARDecoder(
                latent_dim=latent_dim,
                sample_rate=sample_rate,
                **latent_spar_config.get("config", {}),
            )
