...
def vae_sample_grouped(mean, scale, n_w):
    """Same reparam/KL as ``vae_sample`` but also returns the KL split into a
    W-group (first ``n_w`` latent channels) and a spatial-group (the rest).

    This lets the training objective allocate the coding *rate* asymmetrically:
    a lower KL weight on the W-group buys it more bits (higher omni fidelity,
    toward a mono codec), while a higher KL weight on the spatial-group forces a
    compact directional residual (DirAC / MPEG-H style downmix+side-info). No new
    parameters, and the sampled latent is identical to the plain VAE bottleneck,
    so warm-starting from a plain-``vae`` checkpoint is loss-free.
    """
    stdev = nn.functional.softplus(scale) + 1e-4
    var = stdev * stdev
    logvar = torch.log(var)
    ...
    kl_per_channel = (mean * mean + var - logvar - 1)  # [B, C, T]
    kl = kl_per_channel.sum(1).mean()
    kl_w = kl_per_channel[:, :n_w].sum(1).mean()
    kl_spatial = kl_per_channel[:, n_w:].sum(1).mean()

    return latents, kl, kl_w, kl_spatial

class GroupedVAEBottleneck(Bottleneck):
    """VAE bottleneck that exposes a grouped-KL split for FOA rate allocation.

    Structurally identical to ``VAEBottleneck`` (same chunk/sample/decode, no
    learnable parameters); it only additionally reports ``kl_w`` / ``kl_spatial``
    so the loss config can regularize the two latent groups with different KL
    weights. ``n_w`` is the number of leading latent channels reserved as the
    high-fidelity omni (W) "downmix" subspace.
    """

    def __init__(self, n_w: int = 40):
        super().__init__(is_discrete=False)
        self.n_w = int(n_w)
    ...
    def encode(self, x, return_info=False, **kwargs):
        ...
        x, kl, kl_w, kl_spatial = vae_sample_grouped(mean, scale, self.n_w)

        info["kl"] = kl
        info["kl_w"] = kl_w
        info["kl_spatial"] = kl_spatial
