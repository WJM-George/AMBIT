...
def create_training_wrapper_from_config(model_config, model):
    ...
    if model_type == 'autoencoder':
    ...
    elif model_type == 'sceneplan_p11':
        from .sceneplan_p11 import ScenePlanP11TrainingWrapper

        return ScenePlanP11TrainingWrapper(
            model,
            optimizer_configs=training_config.get("optimizer_configs"),
            planner_loss_group_weights=training_config.get(
                "planner_loss_group_weights"
            ),
            use_ema=training_config.get("use_ema", True),
            ema_beta=training_config.get("ema_beta", 0.999),
            ema_power=training_config.get("ema_power", 0.75),
            ema_update_every=training_config.get("ema_update_every", 1),
            ema_update_after_step=training_config.get(
                "ema_update_after_step", 1
            ),
            log_every_n_steps=training_config.get("log_every_n_steps", 10),
        )
    elif model_type == 'sceneplan_p11_v4':
        from .sceneplan_p11_v4 import ScenePlanP11V4TrainingWrapper

        return ScenePlanP11V4TrainingWrapper(
            model,
            optimizer_configs=training_config.get("optimizer_configs"),
            planner_loss_group_weights=training_config.get(
                "planner_loss_group_weights"
            ),
            transfusion_cot_loss_weights=training_config.get(
    ...
    elif model_type == 'sceneplan_p11_audio_aware_v1':
        from .sceneplan_p11_v4 import ScenePlanP11AudioAwareTrainingWrapper

        return ScenePlanP11AudioAwareTrainingWrapper(
            model,
            optimizer_configs=training_config.get("optimizer_configs"),
            planner_loss_group_weights=training_config.get(
                "planner_loss_group_weights"
        ...
        return DiffusionCondTrainingWrapper(
            ...
            use_ema = training_config.get("use_ema", True),
            ema_beta=training_config.get("ema_beta", 0.999),
            ema_power=training_config.get("ema_power", 3 / 4),
            ema_update_every=training_config.get("ema_update_every", 1),
            ema_update_after_step=training_config.get("ema_update_after_step", 1),
            ...
            sceneplan_speech_active_loss_weight=training_config.get(
                "sceneplan_speech_active_loss_weight", 1.0
            ),
            sceneplan_sound_transient_loss=training_config.get(
                "sceneplan_sound_transient_loss"
            ),
            sceneplan_speech_duration_loss=training_config.get(
                "sceneplan_speech_duration_loss"
            ),
            sceneplan_sound_temporal_difference_loss=training_config.get(
                "sceneplan_sound_temporal_difference_loss"
            ),
            sceneplan_cfg_dropout=training_config.get(
...
def create_demo_callback_from_config(model_config, **kwargs):
    ...
    # Demo generation is opt-in.  Treat a missing or empty block as disabled so
    # production configs can omit it entirely without silently constructing a
    # callback with model-specific defaults.
    demo_config = training_config.get("demo")
    if not demo_config or demo_config.get("enabled", True) is False:
        return None
