...
from .ema import EMA, TrainableParameterEMA
...
from ..data.model_sceneplan import make_sceneplan_cfg_dropout_metadata

from time import time


def _assert_finite_scalar(value: torch.Tensor, message: str) -> None:
    """Fail closed without forcing one CUDA host synchronization per step."""

    if not torch.is_tensor(value) or value.ndim != 0:
        raise RuntimeError(message)
    finite = torch.isfinite(value).reshape(())
...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
            ...
            sceneplan_speech_active_loss_weight: float = 1.0,
            sceneplan_sound_transient_loss: tp.Optional[tp.Dict[str, tp.Any]] = None,
            sceneplan_speech_duration_loss: tp.Optional[tp.Dict[str, tp.Any]] = None,
            sceneplan_sound_temporal_difference_loss: tp.Optional[
                tp.Dict[str, tp.Any]
            ] = None,
            sceneplan_cfg_dropout: tp.Optional[tp.Dict[str, tp.Any]] = None,
    ):
        super().__init__()
        ...
        self.diffusion = model
        # Auxiliary-objective validation below depends on the resolved model
        # objective.  Bind it before parsing those configs; assigning it only
        # near the sampler setup made every enabled clean-latent auxiliary fail
        # during wrapper construction before step zero.
        self.diffusion_objective = model.diffusion_objective
        diffusion_network = self._sceneplan_dit_model()
        self.sceneplan_chunk_moe_enabled = bool(
            getattr(diffusion_network, "sceneplan_chunk_moe_enabled", False)
        )
        self.sceneplan_soft_block_enabled = (
            getattr(diffusion_network, "sceneplan_soft_block", None) is not None
        )
        if self.sceneplan_chunk_moe_enabled and sceneplan_speech_duration_loss:
            if bool(sceneplan_speech_duration_loss.get("enabled", False)):
                raise ValueError(
                    "the no-align chunk-MoE pilot cannot enable the legacy "
                    "speech-duration teacher"
                )
        ...
        if use_ema:
            ...
            conditioner_trainable = [
                parameter
                for parameter in self.diffusion.conditioner.parameters()
                if parameter.requires_grad and parameter.is_floating_point()
            ]
            self.conditioner_ema = (
                TrainableParameterEMA(
                    self.diffusion.conditioner,
                    beta=ema_beta,
                    power=ema_power,
                    update_every=ema_update_every,
                    update_after_step=ema_update_after_step,
                )
                if conditioner_trainable
                else None
            )

        else:
            self.diffusion_ema = None
            self.conditioner_ema = None

        self.mask_loss_weight = mask_loss_weight
        self.sceneplan_speech_active_loss_weight = float(
            sceneplan_speech_active_loss_weight
        )
        if (
            not math.isfinite(self.sceneplan_speech_active_loss_weight)
            or self.sceneplan_speech_active_loss_weight < 1.0
        ):
            raise ValueError(
                "sceneplan_speech_active_loss_weight must be finite and >= 1"
            )

        transient_config = dict(sceneplan_sound_transient_loss or {})
        unknown_transient_keys = set(transient_config) - {
            "enabled",
            "scope",
            "quantile",
            "weight",
        ...
        if unknown_transient_keys:
            raise ValueError(
                "unknown ScenePlan sound transient loss keys: "
                f"{sorted(unknown_transient_keys)}"
            )
        self.sceneplan_sound_transient_enabled = bool(
            transient_config.get("enabled", False)
        )
        self.sceneplan_sound_transient_scope = str(
            transient_config.get("scope", "sound_only")
        )
        self.sceneplan_sound_transient_quantile = float(
            transient_config.get("quantile", 0.8)
        )
        self.sceneplan_sound_transient_weight = float(
            transient_config.get("weight", 3.0)
        )
        self.sceneplan_sound_transient_dilation_frames = int(
            transient_config.get("dilation_frames", 2)
        )
        if self.sceneplan_sound_transient_scope != "sound_only":
            raise ValueError(
                "ScenePlan sound transient loss currently supports only "
                "scope='sound_only'; mixed-scene attribution is not identifiable "
                "from a rendered mixture latent"
            )
        if not (
            math.isfinite(self.sceneplan_sound_transient_quantile)
            and 0.0 < self.sceneplan_sound_transient_quantile < 1.0
        ):
            raise ValueError(
                "ScenePlan sound transient quantile must be finite and in (0, 1)"
            )
        if not (
            math.isfinite(self.sceneplan_sound_transient_weight)
            and self.sceneplan_sound_transient_weight >= 1.0
        ):
            raise ValueError(
                "ScenePlan sound transient weight must be finite and >= 1"
            )
        if self.sceneplan_sound_transient_dilation_frames < 0:
            raise ValueError(
                "ScenePlan sound transient dilation_frames must be non-negative"
            )

        duration_config = dict(sceneplan_speech_duration_loss or {})
        unknown_duration_keys = set(duration_config) - {"enabled", "weight"}
        if unknown_duration_keys:
            raise ValueError(
                "unknown ScenePlan speech duration loss keys: "
                f"{sorted(unknown_duration_keys)}"
            )
        self.sceneplan_speech_duration_enabled = bool(
            duration_config.get("enabled", False)
        )
        self.sceneplan_speech_duration_weight = float(
            duration_config.get("weight", 0.1)
        )
        if not (
            math.isfinite(self.sceneplan_speech_duration_weight)
            and self.sceneplan_speech_duration_weight > 0.0
        ):
            raise ValueError(
                "ScenePlan speech duration loss weight must be finite and positive"
            )

        temporal_config = dict(sceneplan_sound_temporal_difference_loss or {})
        unknown_temporal_keys = set(temporal_config) - {
            "enabled",
            "scope",
            "weight",
            "lags",
        ...
        if unknown_temporal_keys:
            raise ValueError(
                "unknown ScenePlan sound temporal-difference loss keys: "
                f"{sorted(unknown_temporal_keys)}"
            )
        self.sceneplan_sound_temporal_difference_enabled = bool(
            temporal_config.get("enabled", False)
        )
        self.sceneplan_sound_temporal_difference_scope = str(
            temporal_config.get("scope", "sound_only")
        )
        self.sceneplan_sound_temporal_difference_weight = float(
            temporal_config.get("weight", 0.05)
        )
        raw_lags = temporal_config.get("lags", [1, 2, 4])
        if (
            not isinstance(raw_lags, list)
            or not raw_lags
            or any(
            ...
            or len(set(raw_lags)) != len(raw_lags)
        ):
            raise ValueError(
                "ScenePlan sound temporal-difference lags must be unique "
                "positive integers"
            )
        self.sceneplan_sound_temporal_difference_lags = tuple(
            sorted(int(value) for value in raw_lags)
        )
        self.sceneplan_sound_temporal_difference_beta = float(
            temporal_config.get("smooth_l1_beta", 0.1)
        )
        if self.sceneplan_sound_temporal_difference_scope != "sound_only":
            raise ValueError(
                "ScenePlan sound temporal-difference loss currently supports "
                "only scope='sound_only'"
            )
        if not (
            math.isfinite(self.sceneplan_sound_temporal_difference_weight)
            and self.sceneplan_sound_temporal_difference_weight > 0.0
            and math.isfinite(self.sceneplan_sound_temporal_difference_beta)
            and self.sceneplan_sound_temporal_difference_beta > 0.0
        ):
            raise ValueError(
                "ScenePlan sound temporal-difference weight and beta must be "
                "finite and positive"
            )
        if (
            self.sceneplan_sound_temporal_difference_enabled
            and self.diffusion_objective not in {"rectified_flow", "rf_denoiser"}
        ):
            raise ValueError(
                "ScenePlan clean-latent temporal-difference loss currently "
                "requires a rectified-flow objective"
            )
        ...
        sceneplan_cfg_dropout = dict(sceneplan_cfg_dropout or {})
        unknown_cfg_keys = set(sceneplan_cfg_dropout) - {
            "mode",
        ...
        if unknown_cfg_keys:
            raise ValueError(
                "unknown ScenePlan CFG dropout keys: "
                f"{sorted(unknown_cfg_keys)}"
            )
        self.sceneplan_cfg_dropout_mode = sceneplan_cfg_dropout.get(
            "mode", "disabled"
        )
        if self.sceneplan_cfg_dropout_mode not in {"disabled", "independent"}:
            raise ValueError(
                "ScenePlan CFG dropout mode must be disabled or independent"
            )
        self.sceneplan_caption_unknown_prob = float(
            sceneplan_cfg_dropout.get("caption_unknown_prob", 0.0)
        )
        self.sceneplan_structured_unknown_prob = float(
            sceneplan_cfg_dropout.get("structured_unknown_prob", 0.0)
        )
        if any(
            not math.isfinite(value) or not 0.0 <= value <= 1.0
            for value in (
                self.sceneplan_caption_unknown_prob,
                self.sceneplan_structured_unknown_prob,
            )
        ):
            raise ValueError("ScenePlan CFG branch probabilities must be in [0, 1]")
        if self.sceneplan_cfg_dropout_mode == "disabled" and (
            self.sceneplan_caption_unknown_prob != 0.0
            or self.sceneplan_structured_unknown_prob != 0.0
        ):
            raise ValueError("disabled ScenePlan CFG must use zero probabilities")
        if self.sceneplan_cfg_dropout_mode == "independent" and (
            self.sceneplan_caption_unknown_prob <= 0.0
            or self.sceneplan_structured_unknown_prob <= 0.0
        ):
            raise ValueError("independent ScenePlan CFG requires two positive probabilities")
        if self.sceneplan_cfg_dropout_mode == "independent" and self.cfg_dropout_prob:
            raise ValueError(
                "generic model-level CFG dropout must be zero when raw ScenePlan "
                "branches use independent dropout"
            )
        self._sceneplan_cfg_running_counts = [0, 0, 0, 0, 0]
    ...
    def _sceneplan_dit_model(self):
        """Resolve ConditionedDiffusion -> DiTWrapper -> DiffusionTransformer."""

        route = getattr(self.diffusion, "model", None)
        return getattr(route, "model", route)

    def reset_timestep_rng(self, seed: int) -> None:
        """Reset the Sobol scramble after DDP rank and resume step are known."""
    ...
    def _sceneplan_speech_active_mask(
        self,
        metadata,
        *,
        target_length: int,
        ...
        """Return [B,T] ground-truth formal-speech activity supervision."""

        if self.sceneplan_speech_active_loss_weight == 1.0:
            return None
        controls = [
            value.get("sceneplan_44") if isinstance(value, dict) else None
            for value in metadata
        ]
        if any(not isinstance(value, dict) for value in controls):
            raise RuntimeError(
                "speech-active loss weighting requires sceneplan_44 metadata"
            )
        try:
            speech_active = torch.stack(
                [
                    torch.as_tensor(
                        value["speech_active_frame_mask"], dtype=torch.bool
                    )
            ...
            raise RuntimeError(
                "invalid ScenePlan controls for speech-active loss weighting"
            ) from error
        if speech_active.ndim != 2:
            raise RuntimeError(
                "ScenePlan speech supervision must have shape [B,T]"
            )
        if int(speech_active.shape[-1]) != int(target_length):
            speech_active = F.interpolate(
                speech_active[:, None, :].to(torch.float32),
                size=int(target_length),
                mode="nearest",
            )[:, 0, :].to(torch.bool)
        return speech_active

    def _apply_sceneplan_independent_cfg_dropout(self, metadata):
        """Independently sample semantic and structured raw-unknown branches.

        Caption dropout nulls Qwen text plus its event/speech role maps only.
        Structured dropout sets the four activity/source-id tracks and four
        trajectory tracks to their learned unknown state only.  Two independent
        Bernoulli draws expose the model to all four conditioning combinations,
        including the fully unconditional state required by CFG inference.
        """

        if self.sceneplan_cfg_dropout_mode == "disabled":
            return metadata, {
                "caption_unknown_fraction": 0.0,
                "structured_unknown_fraction": 0.0,
                "joint_unknown_fraction": 0.0,
                "full_condition_fraction": 1.0,
            }
        if not metadata:
            raise RuntimeError("cannot apply ScenePlan CFG dropout to an empty batch")
        # Metadata is still CPU-resident here.  Sampling on CPU avoids one GPU
        # synchronization per Python metadata row in the input pipeline.
        ...
        decisions = torch.rand((len(metadata), 2)) < probabilities
        output = []
        for index, row in enumerate(metadata):
            if not isinstance(row, dict):
                raise TypeError("ScenePlan training metadata rows must be dictionaries")
            if "prompt" not in row or "sceneplan_44" not in row:
                raise RuntimeError(
                    "independent ScenePlan CFG requires prompt and sceneplan_44"
                )
            caption_unknown = bool(decisions[index, 0].item())
            structured_unknown = bool(decisions[index, 1].item())
            if not caption_unknown and not structured_unknown:
                conditioned_row = row
            else:
                conditioned_row = make_sceneplan_cfg_dropout_metadata(
                    row,
                    caption_unknown=caption_unknown,
                    structured_unknown=structured_unknown,
                )
            if (
                "source_foa_latent" in row
                and conditioned_row.get("source_foa_latent")
                is not row["source_foa_latent"]
            ):
                raise RuntimeError(
                    "Transfusion Editing CFG dropout must retain the exact "
                    "clean source latent"
                )
            output.append(conditioned_row)
        caption = decisions[:, 0]
        structured = decisions[:, 1]
        joint = caption & structured
        full = ~caption & ~structured
        observed_counts = (
            int(decisions.shape[0]),
            int(caption.sum().item()),
            int(structured.sum().item()),
            int(joint.sum().item()),
            int(full.sum().item()),
        )
        self._sceneplan_cfg_running_counts = [
            current + observed
            for current, observed in zip(
                self._sceneplan_cfg_running_counts, observed_counts
            )
        ]
        return output, {
            "caption_unknown_fraction": float(caption.float().mean().item()),
            "structured_unknown_fraction": float(structured.float().mean().item()),
            "joint_unknown_fraction": float(joint.float().mean().item()),
            "full_condition_fraction": float(full.float().mean().item()),
        }

    def on_train_end(self):
        """Emit an auditable aggregate of the independent CFG sampling law."""

        if self.sceneplan_cfg_dropout_mode != "independent":
            return
        counts = torch.tensor(
            self._sceneplan_cfg_running_counts,
            dtype=torch.long,
        ...
        if not self.trainer.is_global_zero:
            return
        samples, caption, structured, joint, full = [
            int(value) for value in counts.detach().cpu().tolist()
        ]
        if samples <= 0:
            raise RuntimeError("independent ScenePlan CFG observed no samples")
        caption_prob = self.sceneplan_caption_unknown_prob
        structured_prob = self.sceneplan_structured_unknown_prob
        result = {
            "mode": "independent",
            "samples": samples,
            "configured": {
                "caption_unknown_prob": caption_prob,
                "structured_unknown_prob": structured_prob,
            },
            "expected": {
                "caption_unknown_fraction": caption_prob,
                "structured_unknown_fraction": structured_prob,
                "joint_unknown_fraction": caption_prob * structured_prob,
                "full_condition_fraction": (1.0 - caption_prob)
                * (1.0 - structured_prob),
            },
            "observed_counts": {
                "caption_unknown": caption,
                "structured_unknown": structured,
                "joint_unknown": joint,
                "full_condition": full,
            },
            "observed_fractions": {
                "caption_unknown_fraction": caption / samples,
                "structured_unknown_fraction": structured / samples,
                "joint_unknown_fraction": joint / samples,
                "full_condition_fraction": full / samples,
            },
        }
        print(
            "SAT_SCENEPLAN_CFG_DROPOUT_RESULT="
            f"{json.dumps(result, sort_keys=True)}",
            flush=True,
        )

    def _apply_sceneplan_speech_active_weight(
        self,
        loss_full: torch.Tensor,
        loss_mask: tp.Optional[torch.Tensor],
        metadata,
    ) -> tp.Tuple[torch.Tensor, tp.Dict[str, torch.Tensor]]:
        """Emphasize speech frames while preserving each sample's mean weight."""

        speech_active = self._sceneplan_speech_active_mask(
            metadata,
            target_length=int(loss_full.shape[-1]),
        ...
        valid = (
            ...
            else torch.ones_like(speech_active, dtype=torch.bool)
        )
        speech_active = speech_active & valid
        weights = 1.0 + (
            self.sceneplan_speech_active_loss_weight - 1.0
        ) * speech_active.to(loss_full.dtype)
        # Normalize over valid frames per sample.  The requested ratio is kept,
        # while batch-to-batch speech duration does not silently change the
        # optimizer's effective learning rate.
        normalizer = (
            (weights * valid.to(weights.dtype)).sum(dim=-1, keepdim=True)
            / valid.sum(dim=-1, keepdim=True).clamp_min(1).to(weights.dtype)
        ).clamp_min(torch.finfo(weights.dtype).eps)
        weights = torch.where(valid, weights / normalizer, torch.ones_like(weights))
        weighted = loss_full * weights.unsqueeze(1)

        channels = int(loss_full.shape[1])
        speech_count = speech_active.sum() * channels
        other = valid & ~speech_active
        other_count = other.sum() * channels
        metrics = {
            "train/speech_active_fraction": (
                speech_active.sum().to(loss_full.dtype)
                / valid.sum().clamp_min(1).to(loss_full.dtype)
            ).detach(),
            "train/mse_speech_active": (
                torch.where(speech_active.unsqueeze(1), loss_full, 0.0).sum()
                / speech_count.clamp_min(1).to(loss_full.dtype)
            ).detach(),
            "train/mse_other_frames": (
                torch.where(other.unsqueeze(1), loss_full, 0.0).sum()
                / other_count.clamp_min(1).to(loss_full.dtype)
            ).detach(),
        }
        return weighted, metrics

    def _sceneplan_sound_only_rows(
        self,
        metadata,
        *,
        device: torch.device,
    ) -> torch.Tensor:
        """Return the rows for which every rendered source is a sound event.

        A mixture latent does not reveal which source caused a temporal
        derivative.  The causal ablation therefore fails closed and applies
        transient weighting only to sound-only ScenePlans.  Extending this to
        mixed scenes requires source stems or a separately supervised onset
        detector, not a guess from the final mixture.
        """
        ...
        for row in metadata:
            if not isinstance(row, dict):
                raise TypeError("ScenePlan training metadata rows must be dictionaries")
            plan = row.get("model_sceneplan")
            if not isinstance(plan, dict):
                raise RuntimeError(
                    "sound transient weighting requires model_sceneplan metadata"
                )
            sources = plan.get("sources")
            if not isinstance(sources, list) or not sources:
                raise RuntimeError(
                    "sound transient weighting requires a non-empty sources list"
                )
            flags.append(
                all(
                    isinstance(source, dict) and source.get("kind") == "sound"
                    for source in sources
                )
            )
        return torch.tensor(flags, dtype=torch.bool, device=device)

    def _apply_sceneplan_sound_transient_weight(
        self,
        loss_full: torch.Tensor,
        ...
        """Emphasize high temporal-derivative latent frames for sound-only rows.

        Salience is computed from the clean frozen VAE latent, never from the
        noised input or the model prediction.  A per-row quantile avoids using
        one scale threshold across unrelated recordings.  The resulting frame
        weights are normalized to mean one over valid frames, so this ablation
        changes *where* the loss is paid without changing the effective batch
        learning rate.
        """

        if not self.sceneplan_sound_transient_enabled:
            return loss_full, {}
        if diffusion_input.ndim != 3 or loss_full.ndim != 3:
            raise RuntimeError(
                "sound transient weighting expects [B,C,T] latent and loss tensors"
            )
        if diffusion_input.shape[0] != loss_full.shape[0]:
            raise RuntimeError("sound transient latent/loss batch mismatch")
        if int(diffusion_input.shape[-1]) != int(loss_full.shape[-1]):
            diffusion_input = F.interpolate(
                diffusion_input.float(),
                size=int(loss_full.shape[-1]),
                mode="linear",
                align_corners=False,
            ).to(diffusion_input.dtype)

        valid = (
            loss_mask.to(device=loss_full.device, dtype=torch.bool)
            if loss_mask is not None
            else torch.ones(
                (loss_full.shape[0], loss_full.shape[-1]),
                dtype=torch.bool,
                device=loss_full.device,
            )
        )
        eligible = self._sceneplan_sound_only_rows(
            metadata, device=loss_full.device
        )

        # RMS temporal derivative over latent channels.  The first frame has
        # no predecessor and is never declared transient merely because the
        # latent begins there.
        clean = diffusion_input.detach().to(device=loss_full.device, dtype=torch.float32)
        ...
        derivative[:, 1:] = (
            (clean[:, :, 1:] - clean[:, :, :-1]).square().mean(dim=1) + 1.0e-12
        ).sqrt()
        pair_valid = torch.zeros_like(valid)
        pair_valid[:, 1:] = valid[:, 1:] & valid[:, :-1]

        transient = torch.zeros_like(valid)
        for row_index in torch.nonzero(eligible, as_tuple=False).flatten().tolist():
            candidates = derivative[row_index][pair_valid[row_index]]
            if candidates.numel() < 2:
                continue
            threshold = torch.quantile(
                candidates, self.sceneplan_sound_transient_quantile
            )
            # Strict comparison prevents a flat/all-zero latent from turning
            # every frame into a transient through a zero quantile tie.
            transient[row_index] = (
                pair_valid[row_index]
                & (derivative[row_index] > threshold)
                & (derivative[row_index] > 1.0e-6)
            )

        dilation = self.sceneplan_sound_transient_dilation_frames
        if dilation:
            kernel = 2 * dilation + 1
            transient = (
                F.max_pool1d(
                    transient[:, None, :].to(torch.float32),
                    kernel_size=kernel,
                    stride=1,
                    padding=dilation,
                )[:, 0, :]
            ...
            transient &= valid & eligible[:, None]

        weights = 1.0 + (
            self.sceneplan_sound_transient_weight - 1.0
        ) * transient.to(loss_full.dtype)
        normalizer = (
            (weights * valid.to(weights.dtype)).sum(dim=-1, keepdim=True)
            / valid.sum(dim=-1, keepdim=True).clamp_min(1).to(weights.dtype)
        ).clamp_min(torch.finfo(weights.dtype).eps)
        ...
        eligible_valid = valid & eligible[:, None]
        ordinary = eligible_valid & ~transient
        channels = int(loss_full.shape[1])
        transient_count = transient.sum() * channels
        ordinary_count = ordinary.sum() * channels
        metrics = {
            "train/sound_transient_eligible_fraction": (
                eligible.sum().to(loss_full.dtype)
                / max(int(eligible.numel()), 1)
            ).detach(),
            "train/sound_transient_frame_fraction": (
                transient.sum().to(loss_full.dtype)
                / eligible_valid.sum().clamp_min(1).to(loss_full.dtype)
            ).detach(),
            "train/mse_sound_transient": (
                torch.where(transient.unsqueeze(1), loss_full, 0.0).sum()
                / transient_count.clamp_min(1).to(loss_full.dtype)
            ).detach(),
            "train/mse_sound_nontransient": (
                torch.where(ordinary.unsqueeze(1), loss_full, 0.0).sum()
                / ordinary_count.clamp_min(1).to(loss_full.dtype)
            ).detach(),
        }
        return weighted, metrics

    def _sceneplan_sound_temporal_difference_auxiliary(
        self,
        *,
        output: torch.Tensor,
        ...
        """Match multi-scale clean-latent derivatives on sound-only scenes.

        The rejected transient experiment multiplied the base RF error at
        frames selected from the target latent.  That changed the estimator's
        weighting and degraded held-out error.  This replacement leaves the
        base RF MSE untouched and adds a small auxiliary on the *predicted
        clean latent*, x0_hat = x_t - t v_hat.  First differences at several
        lags provide a latent analogue of multi-resolution spectral losses
        while remaining exactly compatible with the frozen VAE target.
        """

        if not self.sceneplan_sound_temporal_difference_enabled:
            zero = output.sum() * 0.0
            return zero, {}
        for name, value in (
            ("output", output),
            ...
            if value.ndim != 3:
                raise RuntimeError(
                    f"sound temporal-difference {name} must have shape [B,C,T]"
                )
        if output.shape != noised_inputs.shape or output.shape != clean_target.shape:
            raise RuntimeError(
                "sound temporal-difference prediction/noise/target shapes differ"
            )
        if timesteps.ndim != 1 or timesteps.shape[0] != output.shape[0]:
            raise RuntimeError(
                "sound temporal-difference timesteps must have shape [B]"
            )

        valid = (
            loss_mask.to(device=output.device, dtype=torch.bool)
            if loss_mask is not None
            else torch.ones(
        ...
        predicted_clean = noised_inputs.to(torch.float32) - (
            timesteps.to(device=output.device, dtype=torch.float32)[:, None, None]
            * output.to(torch.float32)
        )
        target_clean = clean_target.to(device=output.device, dtype=torch.float32)

        lag_losses = []
        metrics: tp.Dict[str, torch.Tensor] = {
            "train/sound_temporal_difference_eligible_fraction": (
                eligible.sum().to(torch.float32)
                / max(int(eligible.numel()), 1)
            ).detach()
        }
        for lag in self.sceneplan_sound_temporal_difference_lags:
            if lag >= int(output.shape[-1]):
                raise RuntimeError(
                    "sound temporal-difference lag exceeds the latent length: "
                    f"{lag} >= {output.shape[-1]}"
                )
            prediction_delta = predicted_clean[:, :, lag:] - predicted_clean[:, :, :-lag]
            target_delta = target_clean[:, :, lag:] - target_clean[:, :, :-lag]
            pair_valid = (
                valid[:, lag:]
                & valid[:, :-lag]
                & eligible[:, None]
            ...
            element_loss = F.smooth_l1_loss(
                ...
                target_delta,
                reduction="none",
                beta=self.sceneplan_sound_temporal_difference_beta,
            )
            expanded_valid = pair_valid[:, None, :].expand_as(element_loss)
            row_count = expanded_valid.sum(dim=(1, 2))
            row_loss = torch.where(
                expanded_valid,
                element_loss,
                torch.zeros_like(element_loss),
            ).sum(dim=(1, 2)) / row_count.clamp_min(1).to(element_loss.dtype)
            supported = row_count > 0
            lag_loss = (
                (row_loss * supported.to(row_loss.dtype)).sum()
                / supported.sum().clamp_min(1).to(row_loss.dtype)
            )
            # Keep an autograd-connected zero on a rank whose local batch has
            # no eligible sound-only row; DDP can then synchronize the same
            # parameter set without conditionally changing the graph.
            lag_loss = torch.where(
                supported.any(), lag_loss, output.sum().to(torch.float32) * 0.0
            )
            lag_losses.append(lag_loss)
            metrics[
                f"train/sound_temporal_difference_lag_{lag}"
            ] = lag_loss.detach()

        auxiliary = torch.stack(lag_losses).mean()
        metrics["train/sound_temporal_difference_aux"] = auxiliary.detach()
        metrics["train/sound_temporal_difference_weighted"] = (
            auxiliary.detach() * self.sceneplan_sound_temporal_difference_weight
        )
    ...
    def configure_gradient_clipping(
        ...
        """Clip fused-AdamW gradients safely when training with BF16.

        Lightning rejects its generic clipping path for optimizers that report
        internal AMP unscaling.  BF16 mixed precision does not use a GradScaler,
        so the gradients are already in their true scale and can be clipped
        directly before ``optimizer.step``.  FP16 remains fail-closed because
        clipping scaled gradients would be incorrect.
        """

        clip_value = float(gradient_clip_val or 0.0)
        handles_unscaling = bool(
            getattr(optimizer, "_step_supports_amp_scaling", False)
        )
        ...
        if scaler is not None:
            raise RuntimeError(
                "fused optimizer gradient clipping is supported only when the "
                "precision plugin has no GradScaler (for example BF16 mixed)"
            )
        ...
        if str(algorithm).lower() == "norm":
            ...
            raise ValueError(
                f"unsupported gradient clip algorithm: {gradient_clip_algorithm!r}"
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ScenePlan CFG is applied to raw categorical/text inputs, before Qwen
        # and local embeddings are computed.  The original metadata remains the
        # source of ground-truth-only speech loss supervision.
        conditioning_metadata, sceneplan_cfg_stats = (
            self._apply_sceneplan_independent_cfg_dropout(metadata)
        )
        conditioning = self.diffusion.conditioner(conditioning_metadata, self.device)
        ...
        model_cfg_dropout_prob = (
            0.0
            if self.sceneplan_cfg_dropout_mode == "independent"
            else self.cfg_dropout_prob
        )
        diffusion_result = self.diffusion(
            noised_inputs,
            t,
            cond=conditioning,
            cfg_dropout_prob=model_cfg_dropout_prob,
            return_alignment_info=self.sceneplan_speech_duration_enabled,
            return_moe_info=self.sceneplan_chunk_moe_enabled,
            **extra_args,
        )
        alignment_info = None
        moe_info = None
        if self.sceneplan_speech_duration_enabled or self.sceneplan_chunk_moe_enabled:
            if (
                not isinstance(diffusion_result, tuple)
                or len(diffusion_result) != 2
                or not isinstance(diffusion_result[1], dict)
            ):
                raise RuntimeError(
                    "enabled ScenePlan auxiliary requires "
                    "(prediction, diagnostic_info) from the DiT"
                )
            output, diagnostic_info = diffusion_result
            if self.sceneplan_speech_duration_enabled:
                alignment_info = diagnostic_info
            else:
                moe_info = diagnostic_info
        else:
            output = diffusion_result
        p.tick("diffusion")
        ...
        log_dict = {
            ...
            'train/semantic_caption_v2_fraction': torch.tensor(
                sum(
                    int(md.get("semantic_caption_compiler_version", 1)) == 2
                    for md in metadata
                )
                / max(1, len(metadata)),
                device=diffusion_input.device,
            ),
            'train/cfg_caption_unknown_fraction': torch.tensor(
                sceneplan_cfg_stats["caption_unknown_fraction"],
                device=diffusion_input.device,
            ),
            'train/cfg_structured_unknown_fraction': torch.tensor(
                sceneplan_cfg_stats["structured_unknown_fraction"],
                device=diffusion_input.device,
            ),
            'train/cfg_joint_unknown_fraction': torch.tensor(
                sceneplan_cfg_stats["joint_unknown_fraction"],
                device=diffusion_input.device,
            ),
            'train/cfg_full_condition_fraction': torch.tensor(
                sceneplan_cfg_stats["full_condition_fraction"],
                device=diffusion_input.device,
            ),
        }
        ...
        mse_loss_unweighted = compute_normalized_mse(
            output,
            targets,
            loss_mask,
        ...
        mse_loss_full, speech_loss_metrics = (
            self._apply_sceneplan_speech_active_weight(
                mse_loss_unweighted,
                loss_mask,
                metadata,
            )
        )
        log_dict.update(speech_loss_metrics)
        mse_loss_full, transient_loss_metrics = (
            self._apply_sceneplan_sound_transient_weight(
                mse_loss_full,
                diffusion_input,
                loss_mask,
                metadata,
        ...
        log_dict.update(transient_loss_metrics)
        ...
        if (
            self.sceneplan_speech_active_loss_weight > 1.0
            or self.sceneplan_sound_transient_enabled
        ):
            unweighted_loss, _, _ = compute_masked_loss(
                mse_loss_unweighted,
                loss_mask,
                self.mask_padding_attention,
                self.mask_loss_weight,
            )
            log_dict["train/mse_unweighted"] = unweighted_loss.detach()
        ...
        if self.sceneplan_speech_duration_enabled:
            ...
            required_alignment_metrics = {
                ...
                "duration_uniform_kl",
                "duration_center_mae",
                "duration_teacher_rows",
                "event_bias_strength",
                "speech_bias_strength",
                "bias_nonzero_fraction",
                "speech_interval_rows",
            }
            missing_alignment_metrics = required_alignment_metrics - set(
                alignment_info
            )
            if missing_alignment_metrics:
                raise RuntimeError(
                    "ScenePlan alignment diagnostics are incomplete: "
                    f"{sorted(missing_alignment_metrics)}"
                )
            duration_loss = alignment_info["duration_kl"]
            _assert_finite_scalar(
                duration_loss,
                "ScenePlan speech duration loss must be one finite scalar",
            )
            loss = loss + (
                self.sceneplan_speech_duration_weight * duration_loss
            )
            log_dict.update(
                {
                    "train/speech_duration_kl": duration_loss.detach(),
                    "train/speech_duration_uniform_kl": alignment_info[
                        "duration_uniform_kl"
                    ],
                    "train/speech_duration_center_mae": alignment_info[
                        "duration_center_mae"
                    ],
                    "train/speech_duration_teacher_rows": alignment_info[
                        "duration_teacher_rows"
                    ].to(torch.float32),
                    "train/sceneplan_event_bias_strength": alignment_info[
                        "event_bias_strength"
                    ],
                    "train/sceneplan_speech_bias_strength": alignment_info[
                        "speech_bias_strength"
                    ],
                    "train/sceneplan_alignment_nonzero_fraction": alignment_info[
                        "bias_nonzero_fraction"
                    ],
                    "train/sceneplan_speech_interval_rows": alignment_info[
                        "speech_interval_rows"
                    ].to(torch.float32),
                }
            )
        ...
        if self.sceneplan_chunk_moe_enabled:
            ...
            required_moe_metrics = {
                ...
                "chunk_count",
                "routed_chunk_evaluations",
                "active_layers",
            }
            missing_moe_metrics = required_moe_metrics - set(moe_info)
            if missing_moe_metrics:
                raise RuntimeError(
                    "ScenePlan chunk-MoE diagnostics are incomplete: "
                    f"{sorted(missing_moe_metrics)}"
                )
            moe_auxiliary_loss = moe_info["auxiliary_loss"]
            _assert_finite_scalar(
                moe_auxiliary_loss,
                "ScenePlan chunk-MoE auxiliary loss must be finite",
            )
            loss = loss + moe_auxiliary_loss
            log_dict.update(
                {
                    "train/moe_auxiliary_loss": moe_auxiliary_loss.detach(),
                    "train/moe_raw_load_balance_loss": moe_info[
                        "raw_load_balance_loss"
                    ],
                    "train/moe_router_entropy": moe_info["router_entropy"],
                    "train/moe_router_entropy_loss": moe_info[
                        "router_entropy_loss"
                    ],
                    "train/moe_conflict_gate_mean": moe_info[
                        "conflict_gate_mean"
                    ],
                    "train/moe_router_max_probability": moe_info[
                        "router_max_probability"
                    ],
                    "train/moe_top1_weight_mean": moe_info[
                        "top1_weight_mean"
                    ],
                    "train/moe_conflict_gate_saturation_fraction": moe_info[
                        "conflict_gate_saturation_fraction"
                    ],
                    "train/moe_conflict_logit_l2": moe_info[
                        "conflict_logit_l2"
                    ],
                    "train/moe_prior_evidence_top1_agreement": moe_info[
                        "prior_evidence_top1_agreement"
                    ],
                    "train/moe_chunk_count": moe_info["chunk_count"],
                    "train/moe_routed_chunk_evaluations": moe_info[
                        "routed_chunk_evaluations"
                    ],
                    "train/moe_active_layers": moe_info["active_layers"],
                }
            )
            for expert_index, fraction in enumerate(moe_info["expert_dispatch"]):
                log_dict[f"train/moe_expert_{expert_index}_dispatch"] = fraction

        if self.sceneplan_soft_block_enabled:
            gate_metrics = (
                self._sceneplan_dit_model().sceneplan_soft_block_gate_metrics()
            )
            if not gate_metrics:
                raise RuntimeError(
                    "enabled ScenePlan soft-block attention produced no gate metrics"
                )
            log_dict.update(
                {
                    f"train/softblock_{name}": value
                    for name, value in gate_metrics.items()
                }
            )

        sound_temporal_loss, sound_temporal_metrics = (
            self._sceneplan_sound_temporal_difference_auxiliary(
                output=output,
                noised_inputs=noised_inputs,
                timesteps=t,
                clean_target=diffusion_input,
                loss_mask=loss_mask,
                metadata=metadata,
            )
        )
        if self.sceneplan_sound_temporal_difference_enabled:
            _assert_finite_scalar(
                sound_temporal_loss,
                "ScenePlan sound temporal-difference loss must be finite",
            )
            loss = loss + (
                self.sceneplan_sound_temporal_difference_weight
                * sound_temporal_loss
            )
        log_dict.update(sound_temporal_metrics)

        log_dict["train/mse_loss"] = mse_loss.detach()
        log_dict["train/loss"] = loss.detach()

        # Keep the most recent scalar objectives available to fail-closed
        # training-health callbacks.  This mirrors the Transfusion wrapper and
        # deliberately stores detached tensors so observability never extends
        # the autograd graph or changes the optimization step.
    ...
    def on_before_zero_grad(self, *args, **kwargs):
        if self.diffusion_ema is not None:
            ...
            if getattr(self, 'use_fsdp', False):
                ...
                self.diffusion_ema.update()
        if self.conditioner_ema is not None:
            self.conditioner_ema.update()

    def ema_conditioner_context(self):
        """Temporarily pair EMA conditioner parameters with the EMA DiT core."""

        if self.conditioner_ema is None:
            return nullcontext(self.diffusion.conditioner)
        return self.conditioner_ema.apply_to(self.diffusion.conditioner)
    ...
    def on_validation_epoch_end(self):
        ...
        for validation_timestep in self.validation_timesteps:
            ...
            validation_metrics[outputs_key] = float(val_loss)
        ...
        validation_metrics['val/avg_loss'] = float(val_loss)

        # Formal Editing DiT runs intentionally use logger=none so a remote
        # experiment service is not a promotion dependency.  Preserve their
        # validation curve in the durable tee'd train.log instead.  The hook is
        # explicit and inert for Generation and every other trainer route.
        if (
            os.environ.get("SAT_EDITING_VALIDATION_JSON_LOG") == "1"
            and self.trainer.is_global_zero
        ):
            print(
                "SAT_EDITING_VALIDATION="
                + json.dumps(
                    {
    ...
    def export_model(self, path, use_safetensors=False):
        online_model = self.diffusion.model
        try:
            if self.diffusion_ema is not None:
                self.diffusion.model = self.diffusion_ema.ema_model
            with self.ema_conditioner_context():
                if use_safetensors:
                    save_file(self.diffusion.state_dict(), path)
                else:
                    torch.save({"state_dict": self.diffusion.state_dict()}, path)
        finally:
            self.diffusion.model = online_model
    ...
    def on_load_checkpoint(self, checkpoint):
        """Backfill conditioner EMA when resuming a pre-conditioner-EMA checkpoint."""

        if self.conditioner_ema is None:
            return
        state = checkpoint.get("state_dict", {})
        prefix = "conditioner_ema."
        if any(key.startswith(prefix) for key in state):
            saved_names = checkpoint.get("conditioner_ema_parameter_names")
            current_names = list(self.conditioner_ema.parameter_names)
            if saved_names is not None and list(saved_names) != current_names:
                raise RuntimeError(
                    "conditioner EMA parameter mapping changed between "
                    f"checkpoint and model: saved={list(saved_names)[:8]} "
                    f"current={current_names[:8]}"
                )
            return

        online = dict(self.diffusion.conditioner.named_parameters())
        for index, name in enumerate(self.conditioner_ema.parameter_names):
            online_key = f"diffusion.conditioner.{name}"
            source = state.get(online_key, online[name].detach())
            state[f"{prefix}{self.conditioner_ema._shadow_name(index)}"] = (
                source.detach().clone()
            )
        step = state.get(
            "diffusion_ema.step",
            torch.tensor([int(checkpoint.get("global_step", 0))], dtype=torch.long),
        )
        state[f"{prefix}step"] = step.detach().reshape(()).clone()
        state[f"{prefix}initted"] = torch.tensor(True)

    def on_save_checkpoint(self, checkpoint):
        if self.conditioner_ema is not None:
            checkpoint["conditioner_ema_parameter_names"] = list(
                self.conditioner_ema.parameter_names
            )
        if self.lora_config is not None:
            checkpoint.clear()
...
class DiffusionCondDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        try:
            ...
            with module.ema_conditioner_context():
