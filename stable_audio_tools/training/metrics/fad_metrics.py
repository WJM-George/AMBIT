...
def _download_file_resumable(url: str, destination: str, *, description: str) -> None:
    """Download with byte-range resume and publish the result atomically.

    Interrupted downloads remain at ``destination + '.tmp'``. A later call
    requests only the missing range when supported; servers that ignore Range
    safely restart the temporary file. The final path is never visible until
    the advertised response length has been written.
    """

    temporary = destination + ".tmp"
    existing = os.path.getsize(temporary) if os.path.isfile(temporary) else 0
    ...
    with requests.get(
        ...
        if response.status_code == 416 and existing:
            ...
            remote_size_text = content_range.rpartition("/")[2]
            if remote_size_text.isdigit() and int(remote_size_text) == existing:
        ...
        expected_size = existing + response_size if response_size else None
        with open(temporary, mode) as file, tqdm(
            total=expected_size,
            initial=existing,
            unit="B",
            unit_scale=True,
    ...
    if expected_size is not None and actual_size != expected_size:
        raise RuntimeError(
            f"incomplete download for {description}: "
            f"expected {expected_size} bytes, found {actual_size}"
        )
    os.replace(temporary, destination)


def _resolve_clap_checkpoint(clap_model):
    """Resolve a CLAP checkpoint without depending on the process cwd.

    A bare model filename uses ``<repo>/load/clap_score`` (or
    ``STABLE_AUDIO_TOOLS_CACHE_DIR/clap_score`` when set).  A path supplied by
    the caller is respected verbatim after expansion and absolutization.
    """
    requested = Path(os.fspath(clap_model)).expanduser()
    model_name = requested.name
    if requested != Path(model_name) or requested.is_absolute():
        checkpoint = requested.absolute()
    else:
        default_cache_root = Path(__file__).resolve().parents[3] / 'load'
        cache_root = Path(
            os.environ.get('STABLE_AUDIO_TOOLS_CACHE_DIR', default_cache_root)
        ).expanduser()
        checkpoint = cache_root.absolute() / 'clap_score' / model_name
...
def load_clap_model(clap_model='630k-audioset-fusion-best.pt', device='cuda'):
    ...
    model_name, clap_path = _resolve_clap_checkpoint(clap_model)
    if model_name not in clap_configs:
        raise ValueError(f'clap_model not implemented: {model_name}')

    cfg = clap_configs[model_name]
    url = cfg['url']
    ...
    lock_path = clap_path.parent / 'clap_init.lock'
    ...
    pkg = load_state_dict(str(clap_path))
