...
def sample_diffusion(
    ...
    **sampler_kwargs
    ...
    Args:
        ...
        cfg_rescale_phi: Blend applied by CFG rescale when ``rescale_cfg`` is
            enabled.  ``0`` is vanilla CFG and ``1`` fully matches the
            conditional prediction standard deviation.  The default ``0.4``
            matches the historical stable-audio rescale setting.
    ...
    # Resolve the legacy boolean into the numeric value consumed by DiT.  Prior
    # to this bridge, callers set rescale_cfg=True but DiT only inspected
    # scale_phi, so the advertised rescale setting was silently ignored.
    explicit_scale_phi = sampler_kwargs.pop("scale_phi", None)
    if explicit_scale_phi is None:
        scale_phi = float(cfg_rescale_phi) if rescale_cfg else 0.0
    else:
        scale_phi = float(explicit_scale_phi)
    if not math.isfinite(scale_phi) or not 0.0 <= scale_phi <= 1.0:
        raise ValueError("CFG rescale phi must be finite and in [0, 1]")
