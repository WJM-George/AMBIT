#!/usr/bin/env python3
"""Materialize an isolated Sound delta on eight GPUs without mutating base P9."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--train-render-root",
        type=Path,
        default=Path("/dev/shm/sceneplan_vggsound_sound_delta_v1"),
    )
...
def main() -> int:
    ...
    for name, path in (("sceneplan-root", sceneplan_root), ("output-root", output_root)):
        try:
            ...
            raise ValueError(f"{name} must stay below {supplement_root}") from error
    if not (DATASET_ROOT / "FROZEN_P9.json").is_file():
        raise RuntimeError("Sound delta P8 requires the frozen base P9")
    ...
    if (
        ready.get("schema") != "stable_audio_tools.model_sceneplan_sound_delta_ready"
        or summary.get("status") != "PASS"
        or int(summary.get("rows", -1)) != args.expected_rows
        or summary.get("base_p9_mutated") is not False
    ):
        raise RuntimeError("Sound delta P7.5 is not a valid P8 gate")
    shards = sorted(
        (sceneplan_root / "train").glob("model-sceneplans-train-*.jsonl")
    )
    if len(shards) != (args.expected_rows + 1023) // 1024:
        raise RuntimeError("Sound delta ScenePlan shard count changed")
    gpus = [int(value) for value in args.gpus.split(",") if value.strip()]
    if len(gpus) != 8 or len(set(gpus)) != 8:
        raise ValueError("Sound delta P8 requires eight distinct GPUs")
    render_root = args.train_render_root.expanduser().resolve(strict=False)
    try:
        render_root.relative_to(Path("/dev/shm"))
    except ValueError as error:
        raise ValueError("transient Sound delta renders must stay under /dev/shm") from error
    ...
    try:
        ...
        atomic_write_json(
            ...
            {
                "schema": "stable_audio_tools.sound_delta_p8_state",
                "schema_version": 1,
                "status": "running",
        ...
        while True:
            ...
            if failed is not None or all(code == 0 for code in codes):
                break
            time.sleep(2.0)
        if failed is not None:
            ...
            raise RuntimeError(f"Sound delta P8 worker failed with code {failed}")
    finally:
    ...
    worker_summaries = [
        json.loads(
            (output_root / "workers" / f"worker-{index:02d}.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    materialized = sorted(
        (output_root / "manifests" / "train").glob("materialized-train-*.parquet")
    )
    rows = sum(pq.read_metadata(path).num_rows for path in materialized)
    quarantines = list((output_root / "quarantine").glob("*/*.json"))
    if (
        rows != args.expected_rows
        or len(materialized) != len(shards)
        or quarantines
        or sum(int(row["rows"]) for row in worker_summaries) != args.expected_rows
    ):
        raise RuntimeError("Sound delta P8 coverage/quarantine gate failed")
    final = {
        "schema": "stable_audio_tools.sound_delta_p8_summary",
        "schema_version": 1,
        "status": "PASS",
        "dataset_contract_revision": 5,
        "rows": rows,
        "sceneplan_shards": len(shards),
